...
uint uart_write(uint id, char *buff, uint len) {
  ...
  for (i = 0; i < len; ++i) {
    while (uart->fr & FR_TXFF) asm volatile("nop");
...
void uart_flush(uint id) {
  ...
  while (!(uart->fr & FR_TXFE)) asm volatile("nop");
}
...
uint16_t uart0_available(void) {
  ...
  ret =
      (UART_RX0_BUFFER_SIZE + UART_RxHead - UART_RxTail) & UART_RX0_BUFFER_MASK;
